  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="flex items-start gap-3">
        <button onClick={() => navigate(-1)} className="mt-1 p-1 rounded hover:bg-accent transition-colors">
      ...
      <div className="flex gap-2">
        {lead.phone && (
          <a href={`tel:${lead.phone}`} className="flex items-center gap-2 rounded-lg border bg-card px-3 py-2 text-sm hover:shadow-sm transition-shadow active:scale-95">
            <Phone className="h-4 w-4 text-primary" />
            {lead.phone}
          </a>
        )}
        {lead.email && (
          <a href={`mailto:${lead.email}`} className="flex items-center gap-2 rounded-lg border bg-card px-3 py-2 text-sm hover:shadow-sm transition-shadow active:scale-95 truncate">
      ...
      <div className="rounded-lg border bg-card p-4">
        ...
        {commLogs.length === 0 ? (
          ...
          <div className="space-y-3">
            {commLogs.map((log) => (
              <div key={log.id} className="text-sm border-l-2 border-border pl-3">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <span className="font-medium">{COMM_TYPE_LABELS[log.type]}</span>
                  <span>·</span>
                  <span>{COMM_DIRECTION_LABELS[log.direction]}</span>
                  <span>·</span>
                  <span>{formatDistanceToNow(new Date(log.created_at), { addSuffix: true, locale: da })}</span>
                </div>
                <p className="mt-0.5 text-foreground">{log.summary}</p>
              </div>
            ))}
          </div>
        )}
